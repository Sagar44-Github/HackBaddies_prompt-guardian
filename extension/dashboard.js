...
function renderLanguageBreakdown(history) {
  ...
  const multiAttacks = history.filter(e => (e?.is_multilingual_attack ?? false) === true);

  if (multiAttacks.length === 0) {
    container.innerHTML = '<div style="color:#475569;padding:24px;text-align:center;font-size:12px;">No multilingual attacks detected this session.</div>';
    return;
  }
  ...
  sorted.forEach((item, i) => {
    ...
    requestAnimationFrame(() => {
      ...
      if (fill) setTimeout(() => { fill.style.width = `${(item.count / maxCount) * 100}%`; }, 100 + i * 60);
    });
  });
}
...
function renderThreatProfiles(history) {
  ...
  if (topThreats.length === 0) {
    container.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:32px;color:#475569;">No threats to analyze. All prompts in this session were safe.</div>';
    return;
  }
  ...
  if (typeof window.renderRadarChart !== 'function') {
    console.warn('[dashboard.js] radar.js not loaded — window.renderRadarChart missing');
    container.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:32px;color:#475569;">Radar chart engine not available.</div>';
    return;
  }
  ...
  topThreats.forEach((entry, i) => {
    ...
    setTimeout(() => {
      ...
      const patternScore = entry.pattern_score !== undefined ? Math.round(entry.pattern_score * 100) : 0;
      const groqScore = entry.groq_score !== undefined ? Math.round(entry.groq_score * 100) : 0;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  chrome.storage.local.get(['pg_history'], (data) => {
    ...
    const multilingual = history.filter(e => (e?.is_multilingual_attack ?? false) === true).length;
    ...
    const multilingualEl = document.getElementById('multilingual-count') || document.getElementById('stat-multilingual');
    if (multilingualEl) {
...
function renderHoneypotEvents(history) {
  ...
  const honeypotEvents = history.filter(e => e.user_action === 'honeypot-tracked' || e.honeypot === true);
  
  // Update stats
  if (hpTotal) hpTotal.textContent = honeypotEvents.length;
  if (hpHighest) {
    const maxRisk = honeypotEvents.length > 0 
      ? Math.max(...honeypotEvents.map(e => e.risk_score || 0)).toFixed(1) + '%'
      : '0%';
    hpHighest.textContent = maxRisk;
  }
  if (hpTypes) {
    const uniqueTypes = new Set(honeypotEvents.map(e => e.attack_type).filter(Boolean));
  ...
  tbody.innerHTML = honeypotEvents.slice(0, 20).map(entry => {
    const attackLabel = entry.attack_type && entry.attack_type !== 'Unknown'
      ? entry.attack_type.replace(/_/g, ' ')
      : 'Unknown';
